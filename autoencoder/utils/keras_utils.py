...
class KCompetitive(Layer):
    ...
    def k_comp_tanh(self, x, topk, factor=6.26):
        ...
        P = (x + tf.abs(x)) / 2  ## all positive neurons
        N = (x - tf.abs(x)) / 2  ## all negative neurons

        ### return top k in the input (every column)
        ### indice is index
        values, indices = tf.nn.top_k(P, topk / 2) # indices will be [[0, 1], [2, 1]], values will be [[6., 2.], [5., 4.]]
        # We need to create full indices like [[0, 0], [0, 1], [1, 2], [1, 1]]
        my_range = tf.expand_dims(tf.range(0, tf.shape(indices)[0]), 1)  # will be [[0], [1]]
        ### tile:input, multiples(repeat times)
        ...
        ### P_tmp = factor * tf.reduce_sum(P - P_reset, 1, keep_dims=True) * values  / tf.reduce_sum(values, 1, keep_dims=True)# alpha 6.26 * pos adder
        ### N_tmp = factor * tf.reduce_sum(-N - N_reset, 1, keep_dims=True) * values2 / tf.reduce_sum(values2, 1, keep_dims=True) # alpha 6.26 * neg adder
        P_tmp = factor * tf.reduce_sum(P - P_reset, 1, keep_dims=True) # alpha 6.26 * pos adder
        N_tmp = factor * tf.reduce_sum(-N - N_reset, 1, keep_dims=True) # alpha 6.26 * neg adder
        ...
        res = P_reset - N_reset  ## combine pos and neg neurons
